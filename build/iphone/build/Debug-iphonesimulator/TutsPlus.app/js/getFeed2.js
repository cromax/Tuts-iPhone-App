...
var win = Titanium.UI.currentWindow,
	rows = [],
	query,
	chosenCategory = Titanium.App.Properties.getString('category');
...
query = (Titanium.App.Properties.getString('site') === 'All')
	...
	: "Select title, encoded, guid, category from rss where url='http://feeds.feedburner.com/" + Titanium.App.Properties.getString('site') + "tuts' and category='" + chosenCategory + "'";

Titanium.Yahoo.yql(query, function(e) {		
	var data = e.data,
		tableView = Titanium.UI.createTableView();
		
		//alert(e.data.item instanceof Array);
	...
	if ( e.data.item instanceof Array ) {
		...
		// If viewing ALL, add a thumbnail with each article.
		if ( Titanium.App.Properties.getString('site') === 'All' ) {
			var siteName = data.item.guid.split('http://');
			siteName = siteName[1].split('.');
			newRow.leftImage = '../images/tiny-' + siteName[0] + '.jpg';
		}
